...
## Files corresponding to the original uploaded scripts

| Original file | Refactored destination |
|---|---|
| `Aortic Disease(1).R` | `01` to `07` + docs |
| `Competing risk model(1).R` | `08_run_competing_risk_models.R` |
| `I700(1).R` | `02_build_aortic_subtypes.R` + `05/07` |
| `I710(1).R` | `02_build_aortic_subtypes.R` + `05/07` |
| `I711(1).R` | `02_build_aortic_subtypes.R` + `05/07` |
| `I713(1).R` | `02_build_aortic_subtypes.R` + `05/07` |
| `Iqt(1).R` | `02_build_aortic_subtypes.R` + `05/07` |
| `senlin_sex(1).R` | `09_plot_sex_stratified_forest.R` |
| `个性化森林图(1).R` | `10_plot_personalized_forest.R` |
| `disease(1).R` | source logic documented in `data_dictionary/` and should be added as a separate preprocessing step if needed |

## Before uploading to GitHub

- Remove any participant-level data from the repository
- Confirm that `config/analysis_config.R` is **not** committed
- Double-check that no paths reveal private local directories
- Add your preferred license
- Add your actual Zenodo / repository DOI once available
- Replace example datasets with either:
  - permitted summary-level data, or
  - a clear statement on where the controlled-access derived dataset can be requested

## Suggested companion repositories / archives for the revision

For a journal revision, the cleanest package is:
1. this public code repository,
2. a persistent archive (for example Zenodo) for the exact code version,
3. a supplementary user guide,
4. a data availability statement describing where the processed dataset can be accessed, and
5. a code availability statement linking the DOI and repository.
