...
death <- fread("D:/科研/UKB/data/death_tongji.csv",
               ...
               stringsAsFactors = FALSE)
# 需要检查的特定诊断编码
values_to_check <- c('I715','I716','I718','I719')
...
df <- read_excel("C:/Users/余00/Desktop/Multimorbidity/data/OPCS4_qt.xlsx")
...
merge2$I715_date <- as.Date(merge2$I715_date)
merge2$I716_date <- as.Date(merge2$I716_date)
merge2$I718_date <- as.Date(merge2$I718_date)
merge2$I719_date <- as.Date(merge2$I719_date)
merge2$AN_date <- pmin(merge2$I715_date,merge2$I716_date,merge2$date,
                       merge2$I718_date,merge2$I719_date,
...
merge2$AN[merge2$I715==1|merge2$I716==1|merge2$I718==1|merge2$I719==1|merge2$death_cause==1|merge2$case==1] <- "1"
...
merge2_3 <- merge(merge2_2,disease,by='eid')
...
# 若需保存
write.csv(PAR_table,file="C:/Users/余00/Desktop/PAF_Iqt.csv", row.names = FALSE)
